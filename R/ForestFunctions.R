...
forest.rma.CN <- function(
    x,
    y, # y is the other model if want to show fixed and random effects estimates
    annotate = TRUE,
    addfit = TRUE,
    addpred = FALSE,
    showweights = FALSE,
  ...
  if (x$int.only) {
    ...
    if (missing(col)) {
      ...
      if (length(col) == 1L)  {
        # if user only specified one value, assume it is for the summary polygon
        col <- c(col, "gray50")
      }
    }
  ...
  } else if (length(annosym) == 3L) {
    annosym <- c(annosym, "-")
  } else if (length(annosym) != 4L) {
  ...
  if (missing(slab)) {
    ...
    if (is.null(slab) || (length(slab) == 1L && is.na(slab))) {
      # slab = NULL or slab = NA can be used to suppress study labels
      slab <- rep("", x$k.all)
    }
