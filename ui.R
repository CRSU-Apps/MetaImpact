         ...
         fluidPage(
           ...
           shinyjs::hidden(    # 'hidden' needed, otherwise everything is initially loaded, then removed (i.e. flashes on load up)
             ...
             div(class = 'page', id = 'page2',
                 ...
                 h4("Currently, the meta-analysis is showing an improvement in visual acuity when receiving RANI, but this is not statistically significant (i.e. the odds ratio crosses the line of no effect)."),
                 br(),
             ...
             div(class = 'page', id = 'page3',
                 fluidRow(
                   ...
                   h4("We're going to keep things simple and assume a trial with two equal arms, therefore, we just need to calculate the total sample size. 
                      The calculator runs many simulations, where for each iteration: a new study is simulated (that is similar to the current evidence base); added to the meta-analysis; and the resulting 'impact' (here we're looking at the p-value) is assessed."),
                 ...
                 fluidRow(conditionalPanel(condition = "input.WalkFreqCalcRun3!=0",
                                  h4("The power to have the desired impact (p-value of 0.15 or less) was calculated for total sample sizes of 250, 500, 750, and 1000.
             ...
             div(class = 'page', id = 'page4',
                 ...
                 h4("Below is a funnel plot of the original studies (outcome effect (odds ratio) versus the standard error (which is related to sample size)). Contours and shaded areas can be added to illustrate areas of the plot that would give the desired impact. Press the button below to add the contours."),
                 br(),
                 ...
                 conditionalPanel(condition = "input.WalkAddContours == 0",
                                  actionButton("WalkAddContours", "Add Significance Contours", class="btn-primary btn-large")),
                 br(),
                 conditionalPanel(condition = "input.WalkAddContours != 0",
                                  h4("We can now see what trial characteristics (i.e. effect size and standard error) would lead to certain impacts on the new evidence base. Details of how these contours are calculated can be found in the paper by Langan et all ",
                                     a(href="https://www.jclinepi.com/article/S0895-4356(11)00327-1/fulltext", "here.")),
                                  h4("Now let's add onto the plot all 100 simulated 'new trials' of sample size 1,000 that were used for the power calculation. Press the button below to add the simulations."),
                                  conditionalPanel(condition = "input.WalkAddSims == 0",
                                                   actionButton("WalkAddSims", "Add Simulations", class="btn-primary btn-large"))),
                 br(),
                 conditionalPanel(condition = "input.WalkAddSims != 0",
                                  h4("We can now see that of the 100 simulated new trials (iterations of the calculator), 22 of them are within the shaded contour indicating the desired impact (a significant effect greater than the null effect (i.e. OR greater than 1), where we've defined significance as p-value of 0.15 or less). Therefore, the power of a new study of sample size 1,000 having the desired impact is 22%."),
                                  h4("Why not select one of the other sample sizes tested and see how the simulated trials fall on the contours."))),
             ...
             div(class = 'page', id = 'page5',
                 ...
                 h4("Instead, a random-effects model assumes that every study has it's own underlying treatment effect, but that each of these underlying effects come from the same distribution (i.e. variation isn't purely due to random error, but the underlying effects are 'similar')."),
                 br(),
                 fluidRow(column(4,
                                 h4("In this example, when we fit a random-effects model, the between study heterogeneity (tau-squared on log-odds scale) was estimated at 0.033 (a fixed-effects model forces this to equal 0). The presence of heterogeneity can infer that a random-effects model may be a better 'fit'."),
             ...
             div(class = 'page', id = 'page6',
                 ...
                 conditionalPanel(condition = "input.WalkFreqCalcRun6 != 0", fluidRow(
                                  column(3, h4("The power is much lower when using a random-effects model! A primary reason for this reduction is that when confidence intervals/p-values are calculated, they include both within- and between-study variation - this is what lead to the wider confidence intervals.
                                               In turn, it is then 'harder' to move such measures to the desired levels. Even with the largest study sizes, the between-study variation is still present and driving this 'conundrum'."),
             ...
             div(class = 'page', id = 'page7',
                 ...
                 h4("With regards to meta-analysis, a main difference between these frameworks is how the between-study heterogeneity is modelled. Under a frequentist framework, it is modelled with a single number. However, under a Bayesian framework, it can be modelled with a distribution."),
             ...
             div(class = 'page', id = 'page8',
                 ...
                 h4("You are now welcome to explore the rest of the application, including uploading your own data, and using further options within the calculator (such as changing the significance threshold and type of impact desired)."))
                                              ...
                                              bsCollapsePanel(title="Extended Funnel Plot", style='success',
                                                              ...
                                                              conditionalPanel(condition = "input.LanganOptions.includes('contour')",
                                                                   div(style="position: absolute; right: 40px;", dropMenu(dropdownButton(label="Significance contours look strange?", circle=FALSE, size='xs'), align='left', arrow=FALSE,
                                                                           ...
                                                                           img(src='PlotLim_edging.png', width=200, align = "center"),
                                                                           ...
                                                                           img(src='PlotLim_triangle.png', width=200, align = "center"),
