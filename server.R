      ...
      bsCollapse(
        ...
        bsCollapsePanel(
          ...
          conditionalPanel(
            ...
            withSpinner(plotOutput(outputId = 'powplot'), type = 6),
            radioButtons(
              inputId = 'powplot_options',
              label = NULL,
              c(
            ...
            fluidRow(
              div(
                ...
                downloadButton(outputId = 'powplot_download', label = "Download power plot")
              ),
              div(style = "display:inline-block; width: 10px;", br()),
              div(
                style = "display: inline-block;",
                radioButtons(inputId = 'powplot_choice', label = NULL, c('pdf', 'png'), inline = TRUE)
              )
            )
          ),
          conditionalPanel(
        ...
        bsCollapsePanel(
          ...
          conditionalPanel(
            condition = "output.SingMult == 'multiple'",
            withSpinner(tableOutput(outputId = "powtable"), type = 6),
            downloadButton(outputId = 'powtable_download', label = "Download (CSV)")
          ),
          conditionalPanel(
            condition = "output.SingMult == 'single'",
            withSpinner(htmlOutput(outputId = "singleresult"), type = 6)
          )
        )
      )
    )
